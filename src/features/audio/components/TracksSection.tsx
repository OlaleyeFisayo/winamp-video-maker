...
function TrackName({ title, onRename }: NameProps) {
  ...
  return (
    <input
      ...
      className="h-7 min-w-0 flex-1 truncate rounded-[2px] border border-transparent bg-transparent px-1 text-[15px] leading-[1.3] text-paper transition-colors duration-100 hover:border-rule focus:border-ash focus:outline-none"
    />
  )
}
  ...
  return (
    ...
    >
      <button
        ...
        className="flex size-6 shrink-0 items-center justify-center rounded-[2px] font-mono text-[13px] leading-none text-ash hover:text-paper focus-visible:outline-2 focus-visible:outline-contrast focus-visible:outline-offset-2"
      >
...
export function TracksSection() {
  ...
  const onPick = (picked: FileList | null) => {
    ...
    if (!ok.length) return
    enqueue({ type: "add", files: ok })
    const project = useProject.getState()
    if (!project.name.trim()) project.setName(stripExt(ok[0].name))
  }
    ...
    <PanelSection
      ...
      action={
        <Button
          variant="link"
          className="h-8 px-2"
          icon={<IconPlus size={16} stroke={1.5} aria-hidden />}
    ...
    >
      ...
      {tracks.length === 0 ? (
        <p className="text-[15px] leading-[1.5] text-ash">
          No tracks yet. Add MP3 or WAV files. They play in this order.
        </p>
      ) : (
      ...
      {skipped && <p className="text-[15px] leading-[1.5] text-paper underline">{SKIPPED}</p>}
    </PanelSection>
  )
}
