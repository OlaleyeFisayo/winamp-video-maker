...
export function Timeline() {
  ...
  const durations = tracks.map((t) => t.duration ?? 0)
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className="relative mt-4 cursor-pointer touch-none select-none overflow-hidden rounded-sm border border-rule focus-visible:outline-2 focus-visible:outline-contrast focus-visible:outline-offset-2"
    >
      ...
      <div className="relative h-9 bg-graphite">
        {tracks.map((t, i) => (
          ...
          >
            ...
            <span className="shrink-0 font-mono text-[11px] leading-none text-ash">{formatTime(t.duration)}</span>
          </div>
        ))}
      </div>
      {head !== null && total > 0 && (
